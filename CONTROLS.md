...
Seventy-two settings, in the eight categories the menu shows. Every one takes
...
| Setting | Range | Default | What it does |
|---|---|---|---|
| World scale | 0.50 – 2.00 | 1.00 | How large the world feels. The mod sizes the world from your own IPD, and Titanfall is not built at exactly the scale that assumes, so people often find everything a little too big and the 3D a little flat. **Lower this if hands and weapons look oversized** — the depth gets stronger as you do, because both come from the same number. |
| Field of view: auto | off / on | off | Work the field of view out from your headset rather than using the number below. On at least one headset it derives a view far wider than the display can show, which costs about a third of the frame drawing world you cannot see. Turn it off to use your own number. |
| Field of view | 0.80 – 2.00 | 1.51 | How wide a view the game renders. Used only when auto above is off. **It does not change how big things look** — the image is submitted with its own geometry, so a wider view shows more of the world at the same size and costs more to draw. Lower it to get frame rate back. |
...
| Jump/crouch deadzone | 0.05 – 0.95 | 0.60 | How far the right stick must go up or down before that axis registers. The name is left over: the vertical axis no longer jumps or crouches, it answers the game's two-option prompts. Jump is **A** and crouch is **B**. |
