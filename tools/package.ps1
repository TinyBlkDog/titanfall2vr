...
# A SEED CACHE FOR QUEST 3, optional and inert unless it matches.
#
# A first launch has no headset cache, so it derives the panel geometry and
# looks badly magnified for the first minute -- KNOWN-ISSUES 8, which every
# new player meets. This file gives a Quest 3 on VDXR that geometry up front.
# The fingerprint includes the per-eye resolution the runtime asks for, so it
# is ignored unless the user's Virtual Desktop quality matches: no harm when
# it does not, and nothing in it is personal to the machine it came from.
$seed = Join-Path $PSScriptRoot 'package\quest3.headset.cache'
if (Test-Path $seed) {
    $optional = Join-Path $root 'optional'
    New-Item -ItemType Directory -Force -Path $optional | Out-Null
    Copy-Item $seed (Join-Path $optional 'titanfall2vr.headset.cache')
} else {
    throw "The Quest 3 seed cache is missing at $seed. It ships in every archive; a build that quietly dropped it would leave every Quest 3 user on the derive path."
}
...
#
# ONE EXEMPTION, and it is narrow on purpose. optional	itanfall2vr.headset.cache
# is the Quest 3 seed: authored in tools\package, reviewed, carrying measured
# geometry and nothing about the machine it came from. Every other cache is
# still refused, including one at the archive root, which is where a real
# recording would land if a build ever picked one up by accident.
$forbidden = Get-ChildItem -Path $root -Recurse -File |
    Where-Object { -not ($_.Name -eq 'titanfall2vr.headset.cache' -and
                         $_.Directory.Name -eq 'optional') } |
