...
// view.world_scale. 1.00 is the separation as derived; larger divides it,
// which makes the world feel bigger. See the note at the top of
// SetWorldScale for why this is a multiplier rather than an absolute.
std::atomic<float> g_worldScale{1.0f};

// THE ONLY VALUE ANY CONSUMER SHOULD USE. The raw atomic is the baseline the
// runtime writes; this is the baseline with the wearer's scale applied, and
// both the stereo offset and SourceUnitsPerMetre have to read the same one or
// head movement and eye separation drift apart -- which is precisely the
// disagreement the derived-scale design exists to prevent.
float EffectiveHalfIpdUnits() {
    const float scale = g_worldScale.load(std::memory_order_acquire);
    const float base = g_halfInterpupillaryUnits.load(std::memory_order_acquire);
    return scale > 0.01f ? base / scale : base;
}
            ...
            if (cameraSized && (offsetSelected || compensate || matchViewmodelFov ||
                ...
                if (g_offsetModeKind == OffsetMode::CameraOrigin) {
                    ...
                    costScope.PauseForForward();
                        ...
                        const float scale = eyeSign * EffectiveHalfIpdUnits() / length;
...
float SourceUnitsPerMetre() {
    ...
    // THE BASE, NOT THE SCALED VALUE, and world scale is a no-op if you get
    // this wrong. Measured 2026-09-12: with both scaled, world scale 1.34 took
    // the eye offset from 1.2541 to 0.9359 units and units/metre from 39.37 to
    // 29.38 -- the separation in MILLIMETRES stayed at 31.85 and the wearer saw
    // no change with either eye closed. Of course: the game's geometry is fixed
    // in units, so rescaling the offset and the metre together renders the same
    // image by definition. Only the ABSOLUTE separation in units can change it.
    //
    // So this stays on the runtime-derived baseline. Head translation and hand
    // placement keep their true metric scale, and world scale moves the stereo
    // separation alone, which is the only thing the eye can act on.
...
// view.world_scale -- how large the world feels.
//
// Stereo separation and world scale are one number: SourceUnitsPerMetre() is
// the separation in units over the wearer's real half-IPD in metres, so the
// same value sets how strong the depth is AND how big everything looks. Two
// reports on release day, both the same shape: "there is 3D, but it is subtle"
// and "the world scale feels too big, the hands and weapons look oversized".
// One value, too small, produces all three complaints.
//
// Applied as a divisor on the runtime-derived baseline so 1.00 is exactly the
// old behaviour and SetRuntimeIpdMetres keeps owning the variable it owned.
void SetWorldScale(float scale) {
    if (!(scale > 0.05f && scale < 20.0f)) return;
    const float previous = g_worldScale.exchange(scale, std::memory_order_acq_rel);
    if (std::fabs(previous - scale) < 0.0005f) return;
    char line[260]{};
    std::snprintf(line, sizeof(line),
        "[TF2VR] world scale %.2f: eye separation %.3f Source units (%.1f mm). Larger makes "
        "the world feel bigger and further; smaller makes it feel smaller and closer.\n",
        static_cast<double>(scale), static_cast<double>(EffectiveHalfIpdUnits()),
        static_cast<double>(EffectiveHalfIpdUnits() * 2.0f * 25.4f));
    Tf2VrLogWrite(line, true);
}

float WorldScale() { return g_worldScale.load(std::memory_order_acquire); }
