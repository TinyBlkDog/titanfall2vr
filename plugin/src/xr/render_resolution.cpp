...
bool ForceRenderResolutionNow(int width, int height, bool windowed) {
    ...
    if (!RunEngineConsoleCommand(command)) {
        Tf2VrLog("[TF2VR] live resolution change NOT applied: the engine command buffer was "
...
void ApplyRenderResolutionIfRequested() {
    ...
    if (!RunEngineConsoleCommand(command)) {
        Tf2VrLog("[TF2VR] render resolution NOT applied: the engine command buffer was unavailable or "
...
void LoadHeadsetCache() {
    ...
    if (file == INVALID_HANDLE_VALUE) {
        Tf2VrLog("[TF2VR] headset cache: EMPTY. No headset has been seen yet, so this launch "
...
bool DerivedRenderTarget(int* width, int* height, float scale) {
    ...
    if (g_minAspectFloor && g_engineLetterboxRead && g_engineLetterboxGoal > aspect) {
    ...
    if (g_derivedSetupApplied) {
        ...
        if (g_fovAuto && !g_fovScaleAppliedLate && g_activeHeadset >= 0) {
            ...
            if (late > 0.01f) {
                ...
                Tf2VrLog("[TF2VR] derived setup: FOV scale applied LATE, from the tick, now that the "
