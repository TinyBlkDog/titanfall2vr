...
constexpr ActionEntry kActions[] = {
    ...
    // BOUND, unlike everything else in this table, and only in a diagnostic
    // build: two people reported opposite world-scale errors on release day
    // and the fastest way to settle a number is to hold the headset on and
    // sweep it. F3 and F6 are two of the four fixed experiment keys -- bare,
    // because a modifier fires the game's binding for the modifier too.
    {Action::IpdSmaller,                 "ipd.smaller",             {VK_F3, 0, false}},
    {Action::IpdLarger,                  "ipd.larger",              {VK_F6, 0, false}},
...
// Every `set` a config file actually applied, for the summary line at the end
// of ParseConfigFile. The shipped ini is entirely commented, so anything that
// lands here is something a person typed.
char g_appliedNames[1024]{};
int g_appliedNamesUsed = 0;
...
void ApplyValue(const char* name, float value) {
    ...
    if (std::strcmp(name, "view.world_scale") == 0) { SetWorldScale(value); return; }
...
void ParseConfigFile(const char* path) {
    ...
    while (std::fgets(raw, sizeof(raw), file)) {
        ...
        *space = '\0';
        ...
        if (std::strcmp(left, "bind") == 0) {
            ...
            ++applied;
            if (g_appliedNamesUsed >= 0 &&
                g_appliedNamesUsed < static_cast<int>(sizeof(g_appliedNames)) - 1) {
                const int wrote = std::snprintf(
                    g_appliedNames + g_appliedNamesUsed,
                    sizeof(g_appliedNames) - static_cast<size_t>(g_appliedNamesUsed),
                    "%s%s=%s", g_appliedNamesUsed ? ", " : "", subject, right);
                if (wrote > 0) g_appliedNamesUsed += wrote;
                if (g_appliedNamesUsed >= static_cast<int>(sizeof(g_appliedNames))) {
                    g_appliedNamesUsed = static_cast<int>(sizeof(g_appliedNames)) - 1;
                }
            }
        }
    ...
    std::fclose(file);
    char line[MAX_PATH + 1200]{};
    std::snprintf(line, sizeof(line),
                  "[TF2VR] config: %u directives applied from %s (content %08X)%s%s.\n",
                  applied, path, hash, g_appliedNamesUsed ? " -- " : "",
                  g_appliedNamesUsed ? g_appliedNames : "");
...
float GetWorldScaleV()    { return WorldScale(); }
float GetFovAutoV()       { return FovAuto() ? 1.0f : 0.0f; }
float GetGameFovScaleV()  { return GameFovScale(); }
...
const Setting kSettings[] = {
    ...
    {"view.world_scale", "World scale",
     "How large the world feels. The mod sizes the world from your own IPD, and Titanfall is not built at exactly the scale that assumes, so people often find everything a little too big and the 3D a little flat. Lower this if hands and weapons look oversized; it strengthens the depth at the same time, because both come from the same number.",
     GetWorldScaleV, 0.50f, 2.00f, 1.00f, kSlider, kLive, SettingCategory::View, nullptr},
    {"game.fov_auto", "Field of view: auto",
     "Work the field of view out from your headset instead of using the number below. On at least one headset it derives a view far wider than the display can show, which costs a third of the frame for world you cannot see. Turn it off to use your own number.",
     GetFovAutoV, 0.0f, 1.0f, 0.0f, kToggle, kLive, SettingCategory::View, nullptr},
    {"game.fov_scale", "Field of view",
     "How wide a view the game renders. Used only when auto above is off. This does NOT change how big things look -- the image is submitted with its own geometry, so a wider view shows more of the world at the same size, and costs more to draw. Lower it if you want the frame back.",
     GetGameFovScaleV, 0.80f, 2.00f, 1.51f, kSlider, kLive, SettingCategory::View, nullptr},
...
int CommitStagedSettings() {
    ...
    for (int i = 0; i < kSettingCount; ++i) {
        ...
        // AND CHECK THAT IT LANDED. The bool was dropped on the floor, so a
        // plugins folder this process cannot write meant every panel change
        // reverted on the next launch with nothing anywhere saying why.
        // Counted, not logged per setting: one failure means they all failed,
        // and 69 identical lines would bury the one that matters.
        if (!PersistSettingToIni(kSettings[i].name, g_stagedValue[i])) ++persistFailures;
    ...
    if (persistFailures) {
        char line[400]{};
        std::snprintf(line, sizeof(line),
            "[TF2VR] menu: %d setting%s COULD NOT BE SAVED to %s. They are live now and will "
            "be gone when you restart. The usual cause is the game being installed under "
            "Program Files, where that folder needs administrator rights.\n",
            persistFailures, persistFailures == 1 ? "" : "s",
            g_configPath[0] ? g_configPath : "the ini");
        Tf2VrLogWrite(line, true);
    }
    return applied;
}
