...
Everything below ships with `v0.1.1`. Each entry says what you will
...
## 17. The view can come up fully zoomed in (OPEN)

**Severity: HIGH** when it happens. The game is unplayable until you change a
setting or restart.

**What you see.** The first mission loads and the picture is zoomed all the way
in. You cannot see enough of the world to move or look around usefully. Backing
out to the main menu does not clear it: the menu is zoomed too.

**Workaround, and try this first.** Open the settings panel by clicking both
thumbsticks, go to **View**, and check **Field of view: auto**. If it is on,
turn it off. If that does not clear it, move **Field of view** down a little at
a time; the change applies immediately.

From v0.1.1 the automatic derivation ships **off**, so a fresh install should
not reach this at all. Someone upgrading who already has it switched on in
their ini will keep it until they turn it off.

**Where it stands.** Reported once, on a Meta Quest 3 through Virtual Desktop,
with a log. SUSPECTED, not confirmed: the automatic field-of-view derivation
produces a value with nothing bounding it against what the headset can display.
On the one headset that has been measured it asked for a view 45 per cent wider
than the display -- and a derivation that can land that far out in one
direction can land far out in the other, which is what a fully zoomed view is.
Two details fit: it appears at the first level load, which is when the scale is
applied, and it survives into the menu, because the value is a game convar that
stays where it was put.

What would settle it is one log from someone it happens to. The line
`game.fov_auto: cl_fovScale set to N` is printed on every run from v0.1.1, so
a report now carries its own answer.
